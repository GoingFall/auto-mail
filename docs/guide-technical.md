...
不参与主链路，故未画出。
> 图里省略了三类节点：`Windows 任务计划` 与 `GUI Worker` 只是入口/编排层的另外
> 两种驱动方式（前者走 `scripts/run.ps1` 调同一个 CLI），`MIME 解析` 是
> `SyncEngine` 内部的一步而非独立依赖，`threads / digest / audit / stats`
> 是旁路工具且不参与主链路。细节见下方职责表与目录结构。
...
实线是**主链路**，虚线是**反馈与旁路**。图里合并掉的三处细节：

* **同步取信方式**：`EXAMINE`（只读选中）+ `BODY.PEEK[]`（不置已读标志），全程不发 STORE。
* **`events.status` 的中间态**都落在同一个 `events` 表，上图合并为 `DB` 一个节点。
* **旁路工具**（`audit`、`digest`、`threads`）只读库、写本地文件，不参与主链路。
...
`pushed` 上还挂着三条同类转移（手改 → `externally_modified`、双方都改 →
`conflict`、`archive` → `cancelled`），与上图 `approved` 出发的三条对应。
...
`uncertain` 与 `superseded` 在枚举、CHECK 约束与过滤条件里都支持，但**没有任何
代码路径写入**：反查失败目前按 `push_failed` 处理，v1 也不比较 ICS `SEQUENCE`。
两者是给后续版本预留的槽位，接入时不需要迁移。
...
各阶段与外部系统的具体调用（同步用 `EXAMINE` + `BODY.PEEK`、推送先按
`auto_mail_key` 反查再写入）见上文架构图与数据流图。
...
首批 89 封真实邮件恰好全已过期，因此只证明了「不会做危险动作」；后来一封真实的
未来事件邀请函补上缺口，三个事件全部命中，同时修掉 8 个缺陷。整体召回率**未验证**。
详见 [真实数据验证](eval-real-data.md) 与 [合成语料评测](eval-report.md)。
...
| 现实 | 应对 |
|---|---|
| 无 IDLE，只能轮询 | 同步不是实时的；间隔 ≥ 15 分钟 |
| 无服务端 THREAD | 线程在客户端重建 |
| 认证后必须发 IMAP `ID`，否则 `SELECT` 报 `Unsafe Login` | 自动发送；被拒时记录原始响应、重发 ID 再重连，不误判成授权码错误 |
| Coremail 声明 `SASL-IR` 却拒绝 inline 形式 | 走明文 `LOGIN`，天然规避 |
| RFC 3501 §6.4.8：`559:*` 始终含最后一封（即使 559 高于已分配 UID） | 增量搜索前过 **UIDNEXT 预判门**，否则追平后反复重取最后一封 |

**风控层**：会静默阻断（收到「阻止了一次不安全的收信请求」告警邮件）。
程序用单连接 + 指数退避（1→2→4→…→30 分钟）降低概率。

**功能边界**：v1 不展开 `RRULE`。重复事件按单次写入，标题加「(重复)」，描述保留
原 RRULE；不承诺完整重复事件管理、RSVP 跟踪、与会者同步。
...
曾同时踩中「没传日历后端」与「界面不读阶段结果」：同步与抽取都真做了、日历里
什么都没有，界面却显示干净的「完成」。

现在「立即同步」走 `AppState.run_pipeline()`，它经 `calendar/factory.py`
（与命令行共用）选定后端并传进 `Pipeline`，阶段结果交回总览「本次运行」区。
未授权时退回内存后端会**明确报出原因**，不再显示「完成」。

`factory.py` 返回的不只是后端对象，还有 `CalendarSelection`（实际选了哪个、为什么）。
仅凭「没抛异常」无法区分真实日历与内存后端，因此判定必须显式携带。

顺带修掉：`recent_runs` 读错字段名（`r.id` 应为 `run_id`），
导致「最近运行」永远空白且不报错。详见 [图形界面规格 §16](spec-gui.md)。
